...
const DEFAULT_EXTRA = 300; // assumed total when a product has no custom.extra set
...
export async function buildTaxInvoicePdfData(
  ...
  await Promise.all(
    rawItems.map(async (li) => {
      ...
      // Products with no custom.extra default to 300 SAR (shipping = 300-70 = 230).
      const extra = raw ?? DEFAULT_EXTRA;
