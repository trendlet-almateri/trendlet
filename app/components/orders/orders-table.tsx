...
export function OrdersTable({ orders, onOpenDrawer }: OrdersTableProps) {
  ...
  return (
    ...
    <div className="overflow-x-auto overflow-y-visible rounded-[var(--radius)] border border-[var(--line)] bg-[var(--panel)] shadow-[var(--shadow-sm)]">
      {/* Column proportions come from the shared TableColGroup scale (table-fixed
          renders them; md-only columns collapse below md). */}
      <table className="w-full table-fixed border-collapse text-[13px]">
        <TableColGroup
          cols={[
            { size: "sm" }, // Order
            { size: "lg" }, // Customer
            { size: "md", mdOnly: true }, // Sub-orders
            { size: "md" }, // Status
            { size: "md", mdOnly: true }, // Total
            { size: "xs", mdOnly: true }, // Quantity
            { size: "md", mdOnly: true }, // Alerts
          ]}
        />
