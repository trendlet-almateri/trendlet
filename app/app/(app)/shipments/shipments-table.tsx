...
export function ShipmentsTable({ rows }: { rows: ShipmentRow[] }) {
  ...
  // Mobile: open bottom sheet + preload DHL detail
          ...
          {rows.map((s) => {
            ...
            return (
              <React.Fragment key={s.id}>
                <tr
                  onClick={() => {
                    if (window.innerWidth < 768) openMobile(s);
                    else toggle(s);
                  }}
                  className={cn(
      ...
      >
        ...
        <div className="flex flex-col divide-y divide-[var(--line)]">
          <DetailRow label="Status">
            <span className={cn("pill border", STATUS_PILL[s.status] ?? STATUS_PILL.unknown)}>{s.status.replace("_", " ")}</span>
          </DetailRow>
          <DetailRow label="Shipped"><span className="text-[13px] text-[var(--ink)]">{s.shipped_at ? fmt(s.shipped_at) : "—"}</span></DetailRow>
          <DetailRow label="Type"><span className="text-[13px] capitalize text-[var(--ink)]">{s.shipment_type}</span></DetailRow>
          <DetailRow label="Carrier"><span className="text-[13px] text-[var(--ink)]">{s.carrier?.display_name ?? "—"}</span></DetailRow>
          <DetailRow label="Route"><span className="text-[13px] text-[var(--ink)]">{s.origin ?? "?"} → {s.destination ?? "?"}</span></DetailRow>
          {s.delivered_at && (
            <DetailRow label="Delivered"><span className="text-[13px] text-[var(--ink)]">{fmt(s.delivered_at)}</span></DetailRow>
          )}
        </div>
        ...
        {s.tracking_number && (
          <div className="px-4 py-4">
            <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.4px] text-[var(--muted)]">Tracking Events</p>
                ...
                {d.events.map((e, i) => (
                  <li key={i} className="relative flex gap-3 pb-4 last:pb-0">
                    ...
                    <div className="flex-1 pb-1">
                      ...
                      <p className="text-[11px] text-[var(--muted)]">{fmt(e.timestamp)}{e.location ? ` · ${e.location}` : ""}</p>
                    </div>
                  </li>
                ))}
                {d.events.length === 0 && <li className="text-[12px] text-[var(--muted)]">No tracking events yet.</li>}
              </ol>
            )}
          </div>
        )}
